...
def parse_args():
    ...
    parser.add_argument('--split-name', type=str, 
                        help='Name of the split created',
                        default="split1"
                       )
...
def generate_meta_file_for_class_split(split_dir,
    ...
    def write_meta_file(file_path, classes):
        with open(file_path, 'w') as meta_file:
            for cls in classes:
                for idx in range(80*cls, 80*(cls+1)):
                    file_name = f'image_{str(idx).zfill(4)}.jpg'
                    line = f"{file_name} {str(cls)}\n"
...
def main():
    ...
    split_dir = os.path.join(meta_dir, split_name)
    os.makedirs(split_dir, exist_ok=True)

    generate_meta_file_for_class_split(split_dir, train_classes, val_classes,test_classes)
